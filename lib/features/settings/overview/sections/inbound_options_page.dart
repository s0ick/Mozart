    ...
    return Scaffold(
      ...
      body: ListView(
        children: [
          ...
          if (ref.watch(ConfigOptions.serviceMode) == ServiceMode.tun)
            SwitchListTile.adaptive(
              title: const Text("Enable local proxy (SOCKS5/HTTP)"),
              subtitle: const Text("Not recommended — exposes VPN server IP"),
              secondary: const Icon(Icons.warning_amber_rounded),
              value: ref.watch(ConfigOptions.enableMixedProxy),
              onChanged: ref.read(ConfigOptions.enableMixedProxy.notifier).update,
            ),
          if (ref.watch(ConfigOptions.serviceMode) != ServiceMode.tun || ref.watch(ConfigOptions.enableMixedProxy))
